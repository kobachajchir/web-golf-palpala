...
} from '../../src/modules/users/application/use-cases/family-group.use-cases.js';
import {
  createActor,
...
test('removeMemberFromFamilyGroup permite que el titular saque un asociado de su grupo', async () => {
  ...
  const actor = createActor(
    seedUser(manager, 'holder-user', { profileType: 'member', profileId: 'member-1' }),
    { socio: true },
  );
...
test('removeMemberFromFamilyGroup permite que un asociado se salga a si mismo', async () => {
  ...
  const actor = createActor(
    seedUser(manager, 'associate-user', { profileType: 'member', profileId: 'member-2' }),
    { socio: true },
  );
...
test('removeMemberFromFamilyGroup rechaza socios ajenos al grupo', async () => {
  ...
  const actor = createActor(
    seedUser(manager, 'other-user', { profileType: 'member', profileId: 'member-99' }),
    { socio: true },
  );
