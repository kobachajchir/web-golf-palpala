import './employee.use-case.test.js';
import './family-group.use-case.test.js';
import './handicap.use-case.test.js';
import './license.use-case.test.js';
import './member-number.use-case.test.js';
import './member.use-case.test.js';
import './role.use-case.test.js';
import './auth.member-number.test.js';
import './accounting.fee.use-case.test.js';
import './accounting.installment.use-case.test.js';
import './accounting.cash-closure.use-case.test.js';
import './accounting.config.use-case.test.js';
import './accounting.expense.use-case.test.js';
import './accounting.membership-renewal.use-case.test.js';
import './accounting.member-fee-reconciliation.use-case.test.js';
import './accounting.movement.use-case.test.js';
import './accounting.payment.use-case.test.js';
import './accounting.member-fee-batch-payment.use-case.test.js';
import './accounting.payroll-cycle.use-case.test.js';
import './accounting.salary.use-case.test.js';
import './tournament.registration.use-case.test.js';
