...
import {
  ...
  SYSTEM_ACTOR_UID as ACCOUNTING_SYSTEM_ACTOR_UID,
...
import type { EmployeeDocument, UserDocument } from '../modules/users/domain/models.js';
import {
...
const REPORT_PATH = resolve(
  process.env.RESET_OPERATIONAL_DATA_REPORT_PATH
...
type EmployeeCleanupResult = {
  keptEmployeeId: string;
  deletedEmployeeIds: string[];
  disabledLinkedUserIds: string[];
...
async function deleteCollection(collectionName: string, execute: boolean): Promise<number> {
  if (!execute) {
...
async function resetMemberRenewals(execute: boolean): Promise<number> {
  const firestore = getFirestore();
...
async function getOrCreateAdminAuthUser(existingUid: string | null, password: string) {
  const auth = getAuth();
...
async function cleanupEmployees(execute: boolean): Promise<EmployeeCleanupResult> {
  const firestore = getFirestore();
  ...
  const employees = employeesSnapshot.docs.map((entry) => ({
    id: entry.id,
    ref: entry.ref,
    data: entry.data() as EmployeeDocument,
  }));
  ...
  const existingAdminUser = adminUserSnapshot.exists ? (adminUserSnapshot.data() as UserDocument) : null;
  ...
  adminBatch.set(
    firestore.collection(USERS_COLLECTIONS.employees).doc(keptEmployeeId),
    {
