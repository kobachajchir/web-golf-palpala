...
const REPORT_PATH = resolve(
  process.env.MEMBER_USERS_SEED_REPORT_PATH ?? resolve(process.cwd(), 'seed-reports', 'member-auth-users-seed-report.json'),
);
...
function memberDisplayName(member: MemberDocument): string {
...
async function run() {
  ...
  const members = membersSnapshot.docs
    .map((snapshot) => ({ id: snapshot.id, ...(snapshot.data() as MemberDocument) }))
  ...
  const report = {
    ...
    createdAuthUsers: 0,
    updatedUsers: 0,
    skipped: [] as Array<{ memberId: string; reason: string }>,
    linked: [] as Array<{
      memberId: string;
      memberNumber: string;
      uid: string;
  ...
  for (const member of members) {
    ...
    const authUser = await upsertAuthUser(
      ...
      member.linkedUserId ?? identifierData?.uid ?? null,
    );
