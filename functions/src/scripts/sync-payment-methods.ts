...
import type { PaymentMethodDocument } from '../modules/accounting/domain/models.js';
import {
  FirestoreAccountingTransactionManager,
  SystemClock,
} from '../modules/accounting/infrastructure/firestore/repositories.js';
...
const REPORT_PATH = resolve(
  process.env.SYNC_PAYMENT_METHODS_REPORT_PATH
...
type MethodChange = {
  ...
  current: {
    name: string;
    bancarizado: boolean;
    specialReportingType: string | null;
    active: boolean;
    sortOrder: number;
  } | null;
  next: {
...
function comparableMethod(method: Pick<PaymentMethodDocument, 'name' | 'bancarizado' | 'specialReportingType' | 'active' | 'sortOrder'>) {
  return {
    name: method.name,
...
function methodsMatch(
  current: Pick<PaymentMethodDocument, 'name' | 'bancarizado' | 'specialReportingType' | 'active' | 'sortOrder'>,
  next: Pick<PaymentMethodDocument, 'name' | 'bancarizado' | 'specialReportingType' | 'active' | 'sortOrder'>,
) {
