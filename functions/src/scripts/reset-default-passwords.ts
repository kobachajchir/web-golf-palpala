...
import type { EmployeeDocument, MemberDocument, UserDocument } from '../modules/users/domain/models.js';
...
const REPORT_PATH = resolve(
  process.env.RESET_DEFAULT_PASSWORDS_REPORT_PATH
...
type ResetReport = {
  ...
  inspectedEmployees: number;
  updatedAuthUsers: number;
  createdAuthUsers: number;
  updatedFirestoreUsers: number;
  updatedMemberIdentifiers: number;
  updatedEmployeeLinks: number;
  failedUsers: Array<{ key: string; displayName: string; email: string; reason: string }>;
  confirmationRequiredForExecution: string;
  reportPath: string;
};
...
async function listCollectionDocuments<T>(collectionName: string, limit: number) {
...
function isMemberLoginActive(member: MemberDocument): boolean {
  return member.status !== 'inactive' && member.status !== 'suspended';
}

function getEmployeeCode(employeeId: string, employee: EmployeeDocument): string {
  return employee.employeeCode?.trim() || `E${employeeId.replace(/[^a-zA-Z0-9]/g, '').slice(0, 8).toUpperCase()}`;
}

function getDefaultEmployeeRoles(employeeCode: string) {
...
async function ensureMemberAccess(params: {
  ...
  try {
    ...
    const identifier = identifierSnapshot.exists
      ? (identifierSnapshot.data() as { uid?: string | null; memberId?: string | null })
      : null;
...
async function ensureEmployeeAccess(params: {
  ...
  try {
    ...
    const roleIds = Array.from(
      new Set(existingUser?.roleIds?.length ? existingUser.roleIds : getDefaultEmployeeRoles(employeeCode)),
    );
    const claimsVersion = (existingUser?.claimsVersion ?? 0) + 1;
...
async function run() {
  ...
  const [users, members, employees] = await Promise.all([
    listCollectionDocuments<UserDocument>(USERS_COLLECTIONS.users, options.limit),
    listCollectionDocuments<MemberDocument>(USERS_COLLECTIONS.members, options.limit),
    listCollectionDocuments<EmployeeDocument>(USERS_COLLECTIONS.employees, options.limit),
  ]);
