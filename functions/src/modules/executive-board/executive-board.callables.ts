...
type UpsertBoardMemberInput = {
  ...
  order: number;
  active: boolean;
  appointedAt?: Date | null | undefined;
  endedAt?: Date | null | undefined;
};

type ClaimsSyncTarget = {
  uid: string;
  roleIds: string[];
...
function parseUpsertTermInput(payload: unknown): UpsertTermInput {
...
function parseUpsertBoardMemberInput(payload: unknown): UpsertBoardMemberInput {
  const data = assertIsRecord(payload);
...
function timestampOrNull(value: Date | null | undefined) {
...
function normalizeRoleIds(value: unknown): string[] {
  ...
  return Array.from(new Set(value.filter((roleId): roleId is string => typeof roleId === 'string' && roleId.trim().length > 0)));
}

function userDataFromSnapshot(snapshot: DocumentSnapshot): UserDocument {
...
function buildRoleUpdate(params: {
  ...
  roleIds: string[];
  actorUid: string;
}): { payload: Record<string, unknown>; target: ClaimsSyncTarget } {
...
async function syncClaims(targets: ClaimsSyncTarget[]): Promise<void> {
  const uniqueTargets = new Map<string, ClaimsSyncTarget>();
...
export const executiveBoardUpsertBoardMember = onCall(async (request) => {
  try {
    ...
    const syncTargets = await db.runTransaction(async (transaction) => {
      ...
      const nextSyncTargets: ClaimsSyncTarget[] = [];
      const pendingUserUpdates: Array<{ ref: DocumentReference; payload: Record<string, unknown> }> = [];

      if (previousUid && previousUid !== input.uid && previousWasActive) {
        const previousBoardSeats = await transaction.get(
          db
        ...
        if (!hasOtherActiveSeat) {
          ...
          if (previousUserSnapshot.exists) {
            const previousUser = previousUserSnapshot.data() as UserDocument;
  ...
  try {
    ...
    const syncTargets = await db.runTransaction(async (transaction) => {
      ...
      if (uid && boardMember?.active !== false) {
        const activeSeats = await transaction.get(
          db
            .collection(EXECUTIVE_BOARD_MEMBERS_COLLECTION)
            .where('uid', '==', uid)
            .where('active', '==', true),
