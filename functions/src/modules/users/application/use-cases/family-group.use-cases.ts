...
function actorHasStaffClaims(actor: Actor): boolean {
...
function getActorMemberId(actor: Actor): string | null {
...
export interface AddMemberToFamilyGroupInput {
  groupId: string;
  memberId: string;
}

export interface RemoveMemberFromFamilyGroupInput {
  groupId: string;
  memberId: string;
  replacementHolderMemberId?: string | undefined;
}
...
export async function createFamilyGroupUseCase(params: {
  ...
  return params.transactions.runInTransaction(async (dataAccess) => {
    assertCondition(
      params.input.memberIds.includes(params.input.holderMemberId),
      'failed-precondition',
      'El titular debe pertenecer al grupo familiar.',
    );
...
export async function addMemberToFamilyGroupUseCase(params: {
  actor: Actor | null;
  input: AddMemberToFamilyGroupInput;
  transactions: UsersTransactionManager;
}): Promise<{ familyGroupId: string }> {
  const actor = ensureStaff(params.actor);
  ...
  return params.transactions.runInTransaction(async (dataAccess) => {
    ...
    await dataAccess.familyGroups.update(
...
export async function removeMemberFromFamilyGroupUseCase(params: {
  actor: Actor | null;
  input: RemoveMemberFromFamilyGroupInput;
  transactions: UsersTransactionManager;
}): Promise<{ familyGroupId: string }> {
  ...
  return params.transactions.runInTransaction(async (dataAccess) => {
    ...
    assertCondition(
      isStaffActor || isHolderManagingGroup || isAssociatedLeavingSelf,
      'permission-denied',
      'No ten̩s permisos para modificar este grupo familiar.',
    );
    ...
    assertCondition(
      nextHolderMemberId && nextMemberIds.includes(nextHolderMemberId),
      'failed-precondition',
      'Debes indicar un titular de reemplazo que pertenezca al grupo.',
    );
...
export function parseCreateFamilyGroupInput(payload: unknown): CreateFamilyGroupInput {
...
export function parseAddMemberToFamilyGroupInput(payload: unknown): AddMemberToFamilyGroupInput {
  const data = assertIsRecord(payload);
...
export function parseRemoveMemberFromFamilyGroupInput(payload: unknown): RemoveMemberFromFamilyGroupInput {
  const data = assertIsRecord(payload);
