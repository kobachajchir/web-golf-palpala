...
import type { Actor, EntityWithId, HandicapChargeDocument } from '../../domain/models.js';
...
export interface CreateHandicapChargeInput {
  memberId: string;
  handicapId?: string | null | undefined;
  period: string;
  collectionAmountMinor: number;
  transferAmountMinor: number;
  associationName: string;
  transferDueDate?: Date | null | undefined;
  notes?: string | null | undefined;
  collectNow?: boolean | undefined;
  paymentMethodId?: string | null | undefined;
  operationDate?: Date | undefined;
}

export interface TransferHandicapToAssociationInput {
  handicapChargeId: string;
  paymentMethodId?: string | null | undefined;
  operationDate?: Date | undefined;
  notes?: string | null | undefined;
}

export interface TransferPendingHandicapToAssociationInput {
...
function buildReceiptNumber(operationDate: Date, movementId: string) {
  const date = operationDate.toISOString().slice(0, 10).replaceAll('-', '');
  return `REC-${date}-${movementId.slice(0, 8).toUpperCase()}`;
}

function readReceiptNumber(metadata: Record<string, unknown> | undefined) {
...
export async function createHandicapChargeUseCase(params: {
  actor: Actor | null;
  input: CreateHandicapChargeInput;
...
export async function transferHandicapToAssociationUseCase(params: {
  actor: Actor | null;
  input: TransferHandicapToAssociationInput;
  transactions: AccountingTransactionManager;
}): Promise<{ handicapChargeId: string; expenseMovementId: string; duplicate: boolean }> {
  const actor = ensureStaff(params.actor);
...
export async function transferPendingHandicapToAssociationUseCase(params: {
  actor: Actor | null;
  input: TransferPendingHandicapToAssociationInput;
  transactions: AccountingTransactionManager;
}): Promise<{ expenseMovementId: string; handicapChargeIds: string[]; amountMinor: number; duplicate: boolean }> {
  const actor = ensureStaff(params.actor);

  return params.transactions.runInTransaction(async (dataAccess) => {
    const associationName = params.input.associationName?.trim() || 'AAG';
    const associationKey = normalizeAssociationName(associationName);
    const collectedCharges: Array<EntityWithId<HandicapChargeDocument>> = [];
    ...
    const pendingCharges = collectedCharges.filter((charge) =>
      !charge.expenseMovementId
    ...
    await Promise.all(pendingCharges.map((charge) => dataAccess.handicapCharges.update(
      charge.id,
      {
        expenseMovementId: movement.movementId,
        status: 'closed',
...
export function parseCreateHandicapChargeInput(payload: unknown): CreateHandicapChargeInput {
...
export function parseTransferHandicapToAssociationInput(payload: unknown): TransferHandicapToAssociationInput {
  const data = assertIsRecord(payload);
...
export function parseTransferPendingHandicapToAssociationInput(payload: unknown): TransferPendingHandicapToAssociationInput {
  const data = assertIsRecord(payload);
