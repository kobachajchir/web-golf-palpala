...
import {
  ...
  getMercadoPagoCheckoutStatusUseCase,
  parseCreateMercadoPagoCheckoutInput,
  parseGetMercadoPagoCheckoutStatusInput,
  parseMercadoPagoPaymentResponse,
  reconcileMercadoPagoPaymentsUseCase,
  recordMercadoPagoEventUseCase,
...
} from '../application/use-cases/mercado-pago.use-cases.js';
...
function pickMercadoPagoHeaders(headers: Record<string, string | string[] | undefined>): Record<string, string> {
  return {
...
function createMercadoPagoClient(): MercadoPagoClient {
  const accessToken = readEnv('MP_ACCESS_TOKEN');
  const useMock = isEmulatorRuntime() && !accessToken;

  return {
    async createPreference(payload: MercadoPagoPreferencePayload): Promise<MercadoPagoPreferenceResult> {
      if (useMock) {
