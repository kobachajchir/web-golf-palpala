...
import { editFinancialMovementUseCase, setFinancialMovementBalanceInclusionUseCase, voidFinancialMovementUseCase, } from '../../src/modules/accounting/application/use-cases/movement.use-cases.js';
import { InMemoryAccountingTransactionManager, createAccountingActor, seedIncomeCategory, seedPaymentMethod, } from '../helpers/accounting-fakes.js';
function createAdministrativeActor() {
    return createAccountingActor('admin-1', ['administrativo'], { administrativo: true });
}
function seedPostedIncome(manager) {
...
test('administracion puede editar un movimiento y conserva historial auditable', async () => {
    ...
    assert.equal(editHistory[0].previous?.grossAmountMinor, 100_000);
...
//# sourceMappingURL=accounting.movement.use-case.test.js.map