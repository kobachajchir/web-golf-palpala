...
import { parseRegisterExpenseMovementInput, postExpenseMovementUseCase, registerExpenseMovementUseCase, reviewExpenseUseCase, submitExpenseUseCase, } from '../../src/modules/accounting/application/use-cases/expense.use-cases.js';
...
test('egreso exige categoria de egreso valida', async () => {
    ...
    await assert.rejects(() => submitExpenseUseCase({
...
test('rechazar rendicion exige motivo', async () => {
    ...
    await assert.rejects(() => reviewExpenseUseCase({
        actor: createAdminActor(),
...
test('egreso administrativo exige seleccionar una cuenta de origen', () => {
    assert.throws(() => parseRegisterExpenseMovementInput({
        categoryId: FINANCIAL_EXPENSE_CATEGORY_IDS.proveedores,
        amountMinor: 100_000,
    }), /paymentMethodId/i);
});
...
//# sourceMappingURL=accounting.expense.use-case.test.js.map