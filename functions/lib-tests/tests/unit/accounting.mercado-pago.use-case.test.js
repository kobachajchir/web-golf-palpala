...
import { createMercadoPagoCheckoutUseCase, processMercadoPagoPaymentUseCase, } from '../../src/modules/accounting/application/use-cases/mercado-pago.use-cases.js';
import { InMemoryAccountingTransactionManager, createAccountingActor, seedAccountingMember, seedActiveFinancialConfig, seedIncomeCategory, seedPaymentMethod, } from '../helpers/accounting-fakes.js';
const fixedClock = {
...
function seedPendingMemberFeeCharge(manager, memberFeeChargeId, overrides) {
...
function createApprovedPayment(sessionId, overrides) {
    return {
...
test('Mercado Pago no permite incluir cargos ya pagados', async () => {
    ...
    const mercadoPago = createPreferenceClient();
    await assert.rejects(() => createMercadoPagoCheckoutUseCase({
        actor: createAdminActor(),
        input: {
...
//# sourceMappingURL=accounting.mercado-pago.use-case.test.js.map