...
import { accountingCreateHandicapCharge, accountingCreateMacroDebitSettlement, accountingGenerateCuota, accountingGenerateFeePreview, accountingPostExpenseMovement, accountingPostSalaryPayment, accountingReconcileMacroSettlement, accountingRecordExternalReference, accountingRegisterPayment, accountingReviewExpense, accountingSetCreditCommissionRule, accountingSubmitExpense, accountingTransferHandicapToAssociation, accountingUpsertFinancialConfig, accountingUpsertSalaryConfiguration, accountingVoidFinancialMovement, } from '../../src/index.js';
...
function audit(now, actor = 'system') {
    return {
        createdAt: now,
        createdBy: actor,
        updatedAt: now,
...
function callableRequest(data, authData) {
    return {
        data,
        auth: authData,
        rawRequest: { headers: {} },
...
test('todas las callables validan auth', async () => {
    ...
    for (const item of cases) {
        await assert.rejects(() => item.fn.run(callableRequest(item.payload)), expectHttpsError('unauthenticated'));
    }
});
test('todas las callables validan rol', async () => {
    ...
    for (const item of cases) {
        await assert.rejects(() => item.fn.run(callableRequest(item.payload, socioAuth)), expectHttpsError('permission-denied'));
    }
});
test('payload inválido falla con error claro', async () => {
    ...
    for (const callable of cases) {
        await assert.rejects(() => callable.run(callableRequest({}, directivoAuth)), expectHttpsError('invalid-argument'));
    }
});
test('directivo y administrativo pueden modificar cuotas, pero administrativo no controles sensibles', async () => {
    ...
    await assert.rejects(() => accountingUpsertFinancialConfig.run(callableRequest({
        fullMemberFeeMinor: 11000000,
        familyAssociatePctBps: 5000,
        lifetimePctBps: 5000,
        minorPctBps: 3000,
...
test('empleado puede crear su propia expense_submission y no puede aprobar rendición', async () => {
    ...
    await assert.rejects(() => accountingReviewExpense.run(callableRequest({
...
test('postExpenseMovement solo desde approved', async () => {
    const adminAuth = auth('admin-1', { administrativo: true, claimsVersion: 1 });
    await assert.rejects(() => accountingPostExpenseMovement.run(callableRequest({
        expenseSubmissionId: 'expense-1',
    }, adminAuth)), expectHttpsError('failed-precondition'));
});
test('referencias inexistentes fallan', async () => {
    const adminAuth = auth('admin-1', { administrativo: true, claimsVersion: 1 });
    const directivoAuth = auth('directivo-1', { directivo: true, claimsVersion: 1 });
    await assert.rejects(() => accountingGenerateFeePreview.run(callableRequest({ memberId: 'member-missing', period: '2026-04' }, adminAuth)), expectHttpsError('not-found'));
    await assert.rejects(() => accountingCreateHandicapCharge.run(callableRequest({ memberId: 'member-missing', period: '2026-04', collectionAmountMinor: 1000, transferAmountMinor: 1000, associationName: 'AAG' }, adminAuth)), expectHttpsError('not-found'));
    await assert.rejects(() => accountingRecordExternalReference.run(callableRequest({ referenceType: 'F931', period: '2026-04', amountMinor: 1000, linkedMovementId: 'movement-missing' }, directivoAuth)), expectHttpsError('not-found'));
});
test('voidFinancialMovement requiere directivo', async () => {
    const adminAuth = auth('admin-1', { administrativo: true, claimsVersion: 1 });
    await assert.rejects(() => accountingVoidFinancialMovement.run(callableRequest({ movementId: 'movement-posted-1', reason: 'duplicado' }, adminAuth)), expectHttpsError('permission-denied'));
});
...
//# sourceMappingURL=accounting.integration.test.js.map