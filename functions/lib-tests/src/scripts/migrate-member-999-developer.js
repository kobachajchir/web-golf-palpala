...
import { DEVELOPER_ROLE_ID, USERS_COLLECTIONS, } from '../modules/users/domain/constants.js';
...
const TARGET_MEMBER_NUMBER_VARIANTS = ['999', '000999'];
...
const REPORT_PATH = resolve(process.env.MEMBER_999_MIGRATION_REPORT_PATH
...
function appendMigrationNote(current) {
...
function comparableClaims(value) {
    return JSON.stringify(Object.fromEntries(Object.entries(value).sort(([left], [right]) => left.localeCompare(right))));
}
async function waitForUserWriteClaimSync(uid, expectedClaimsVersion) {
    const auth = getAuth();
...
async function findTargetMember() {
    ...
    const snapshots = await Promise.all(TARGET_MEMBER_NUMBER_VARIANTS.map((memberNumber) => firestore.collection(USERS_COLLECTIONS.members).where('memberNumber', '==', memberNumber).get()));
    const candidates = new Map(snapshots.flatMap((snapshot) => snapshot.docs).map((entry) => [entry.id, entry]));
    if (candidates.size !== 1) {
        throw new Error(`La migracion exige exactamente un socio con numero ${TARGET_MEMBER_NUMBER}; se encontraron ${candidates.size}. No se modifico nada.`);
    }
...
async function run() {
    ...
    const charges = [...chargeDocuments.values()].map((entry) => ({
        id: entry.id,
        ref: entry.ref,
        data: entry.data(),
    }));
    const openCharges = charges.filter((entry) => entry.data.status === 'pending' || entry.data.status === 'overdue');
    const unsafeGroupedCharges = openCharges.filter((entry) => entry.data.billingMode === 'single_group_charge'
        || Boolean(entry.data.familyGroupId)
        || Boolean(entry.data.holderMemberId && entry.data.holderMemberId !== member.id));
    const safeOpenCharges = openCharges.filter((entry) => entry.data.billingMode !== 'single_group_charge'
        && !entry.data.familyGroupId
        && (!entry.data.holderMemberId || entry.data.holderMemberId === member.id));
    if (unsafeGroupedCharges.length > 0) {
        throw new Error(`Se detectaron ${unsafeGroupedCharges.length} cuotas grupales vinculadas al socio 999. La migracion se cancela para no afectar a otras personas.`);
    }
    ...
    const chargeChanges = safeOpenCharges.filter((entry) => entry.data.status !== 'exempt'
    ...
    if (options.execute && (plannedChanges.userAccess || plannedChanges.memberExemption || chargeChanges.length > 0)) {
        await firestore.runTransaction(async (transaction) => {
            ...
            if (currentMember.memberNumber !== member.data.memberNumber
...
//# sourceMappingURL=migrate-member-999-developer.js.map