...
const REPORT_PATH = resolve(process.env.MEMBER_FEE_RECONCILIATION_REPORT_PATH
...
async function listPeriods(selectedPeriod) {
    ...
    return [...new Set(snapshot.docs
            ...
            .filter((period) => /^\d{4}-(0[1-9]|1[0-2])$/.test(period)))].sort();
}
async function run() {
...
//# sourceMappingURL=reconcile-member-fee-renewals.js.map