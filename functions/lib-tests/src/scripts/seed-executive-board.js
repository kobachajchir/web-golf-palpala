...
async function getOrCreateBoardAuthUser(member, email, displayName) {
    const auth = getAuth();
    try {
...
async function run() {
    ...
    for (const target of BOARD_TARGETS) {
        ...
        await firestore.runTransaction(async (transaction) => {
            ...
            transaction.set(firestore.collection('executive_board_members').doc(`${TERM_ID}_${target.positionCode}`), {
                termId: TERM_ID,
                uid: authUserResult.user.uid,
                memberId: member.id,
                memberNumber: member.memberNumber,
...
//# sourceMappingURL=seed-executive-board.js.map