...
const REPORT_PATH = resolve(process.env.PARTIAL_PAYMENT_MIGRATION_REPORT_PATH
...
function parseOptions() {
    ...
    if (!planId)
        throw new Error('Debes indicar --plan-id.');
    return {
...
function assertEligiblePlan(params) {
    ...
    if (root.installmentRole !== 'charge')
        throw new Error('El movimiento no es la carga principal de un plan.');
    if (root.metadata?.partialPaymentMode === 'flexible')
        return;
    if (root.status !== 'pending')
        throw new Error('Solo se migra un plan pendiente.');
    ...
    if (linkedPayments.length > 0)
        throw new Error('El plan tiene movimientos vinculados y requiere revision manual.');
}
function summarizePlan(id, root, linkedCount) {
    return {
        id,
...
async function run() {
    ...
    if (options.execute && !alreadyMigrated) {
        await db.runTransaction(async (transaction) => {
            ...
            if (!current || current.metadata?.partialPaymentMode === 'flexible')
                return;
    ...
    const finalRoot = finalSnapshot.data();
    const report = {
        ok: true,
...
//# sourceMappingURL=migrate-installment-plan-to-partial-payments.js.map