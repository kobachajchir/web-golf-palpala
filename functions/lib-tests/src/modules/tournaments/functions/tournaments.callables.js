...
import { approveTournamentRegistrationUseCase, parseApproveTournamentRegistrationInput, parseRecordTournamentRegistrationPaymentInput, parseRegisterExternalTournamentParticipantInput, parseRegisterTournamentParticipantInput, recordTournamentRegistrationPaymentUseCase, registerExternalTournamentParticipantUseCase, registerTournamentParticipantUseCase, } from '../application/use-cases/registration.use-cases.js';
...
import { emitRoleNotification, emitUserNotification, formatAmountMinor, } from '../../notifications/notifications.service.js';
...
function isAppUserId(userId) {
    return userId !== PUBLIC_TOURNAMENT_USER_ID && !userId.startsWith('external:');
}
async function safelyEmitTournamentNotification(functionName, handler) {
    try {
        await handler();
    }
    catch (error) {
...
export const tournamentsApproveRegistration = onCall(async (request) => {
    try {
        ...
        const notificationActorUid = actor?.uid ?? 'system';
