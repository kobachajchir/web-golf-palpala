...
import { assertIsRecord, parseOptionalNullableString, parseOptionalString, parseRequiredString, resolveActor, toHttpsError, } from '../users/application/shared.js';
import { FirestoreUsersTransactionManager, SystemClock } from '../users/infrastructure/firestore/repositories.js';
import { NOTIFICATIONS_COLLECTIONS, actorHasRole, buildActorSnapshot, emitNotification, emitRoleNotification, isStaffActor, resetMemberPasswordFromNotification, } from './notifications.service.js';
const transactions = new FirestoreUsersTransactionManager(new SystemClock());
const STAFF_NOTIFICATION_ROLE_IDS = ['administrativo', 'directivo'];
...
function ensureNotificationAccess(actor) {
...
function ensureNotificationStaff(actor) {
...
function parseOptionalStringArray(data, field) {
    if (!(field in data)) {
        return undefined;
    }
...
function parsePublicInquiryInput(payload) {
...
function parseMemberInquiryInput(payload) {
    const data = assertIsRecord(payload);
    const subject = parseRequiredString(data, 'subject');
    const message = parseRequiredString(data, 'message');
    assertCondition(subject.length <= 140, 'invalid-argument', 'El asunto no puede superar 140 caracteres.');
...
function parseDeliveryId(payload) {
    return parseRequiredString(assertIsRecord(payload), 'deliveryId');
}
function canActorAccessDelivery(actor, delivery) {
...
async function getDeliveryForActor(deliveryId, actor) {
...
export const notificationsMarkRead = onCall(async (request) => {
    try {
        const actor = ensureNotificationAccess(await getActorFromCallableRequest(request.auth));
...
export const notificationsExecuteAction = onCall(async (request) => {
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            if (delivery.deliveryScope === 'shared_role_action') {
                const relatedSnapshot = await transaction.get(db.collection(NOTIFICATIONS_COLLECTIONS.notificationDeliveries)
                    .where('notificationId', '==', delivery.notificationId));
...
export const notificationsListAdmin = onCall(async (request) => {
    try {
        ensureNotificationStaff(await getActorFromCallableRequest(request.auth));
        ...
        const items = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }))
            ...
            .filter((item) => !roleId || item.recipientRoleIds.includes(roleId))
...
//# sourceMappingURL=notifications.callables.js.map