...
import { assertIsRecord, buildCustomClaims, ensureStaff, hasExecutiveAccess, parseRequiredString, parseRequiredStringArray, pickPrimaryRoleId, } from '../shared.js';
...
export async function assignRoleUseCase(params) {
    ...
    return params.transactions.runInTransaction(async (dataAccess) => {
        ...
        for (const internalRoleId of INTERNAL_ROLE_IDS) {
            assertCondition(currentRoleIds.has(internalRoleId) === nextRoleIds.has(internalRoleId), 'permission-denied', 'Los roles internos del sistema no se pueden agregar ni quitar desde administracion.');
        }
        ...
        if (!actorHasExecutiveAccess) {
            ...
            assertCondition(targetUser.profileType === 'member', 'permission-denied', 'Administracion solo puede conceder acceso administrativo a perfiles de socios.');
            assertCondition(removedRoleIds.length === 0, 'permission-denied', 'Administracion no puede quitar roles.');
            assertCondition(addedRoleIds.length > 0 && addedRoleIds.every((roleId) => roleId === 'administrativo'), 'permission-denied', 'Administracion solo puede agregar el rol Administrativo.');
        }
        const assignableRoleIds = params.input.roleIds.filter((roleId) => !INTERNAL_ROLE_IDS.includes(roleId));
