...
import { FINANCIAL_INCOME_CATEGORY_IDS, PAYMENT_METHOD_IDS, SYSTEM_ACTOR_UID, } from '../../domain/constants.js';
import { assertCondition } from '../../domain/errors.js';
import { assertIsRecord, calculateEarlyPaymentDiscount, calculateMembershipRenewalDueDate, ensureAuthenticatedActor, ensureStaff, parseOptionalAmountMinor, parseOptionalNullableString, parseOptionalRecord, parseRequiredEnum, parseRequiredString, toClubAccountingPeriod, } from '../shared.js';
...
function centsToMercadoPagoAmount(amountMinor) {
...
function getActorIsStaff(actor) {
    const isAdministrative = actor.user.roleIds.includes('administrativo') && actor.claims.administrativo === true;
    const isExecutive = (actor.user.roleIds.includes('comite_ejecutivo') || actor.user.roleIds.includes('directivo')) &&
        (actor.claims.comite_ejecutivo === true || actor.claims.directivo === true);
    return isAdministrative || isExecutive;
}
function ensureActorCanPayItem(actor, item) {
    if (getActorIsStaff(actor)) {
        return;
    }
    assertCondition(actor.user.profileType === 'member' && actor.user.profileId && item.memberId === actor.user.profileId, 'permission-denied', 'Solo podés pagar cargos propios desde Mercado Pago.');
}
function buildCheckoutIdempotencyKey(items, actor) {
...
function buildSessionId(idempotencyKey) {
    return `mp_${idempotencyKey.slice(0, 40)}`;
}
function normalizeMercadoPagoSessionStatus(payment) {
...
async function assertMemberCanPay(dataAccess, memberId) {
    ...
    assertCondition(member.status !== 'inactive' && member.status !== 'suspended', 'failed-precondition', 'No se pueden crear pagos para socios dados de baja o suspendidos.');
}
async function resolveCheckoutItem(params) {
    ...
    if (input.sourceType === 'member_fee_charge') {
        ...
        assertCondition(charge.status === 'pending' || charge.status === 'overdue', 'failed-precondition', 'Solo se pueden pagar cuotas pendientes o vencidas.');
...
export async function createMercadoPagoCheckoutUseCase(params) {
    ...
    const prepared = await params.transactions.runInTransaction(async (dataAccess) => {
        ...
        const session = {
            items: resolvedItems,
            memberIds: Array.from(new Set(resolvedItems.map((item) => item.memberId).filter((memberId) => Boolean(memberId)))),
            grossAmountMinor,
            currency: 'ARS',
            status: 'creating',
            preferenceId: null,
    ...
    try {
        ...
        await params.transactions.runInTransaction(async (dataAccess) => {
            await dataAccess.mercadoPagoCheckoutSessions.update(prepared.sessionId, {
                status: 'ready',
...
export async function getMercadoPagoCheckoutStatusUseCase(params) {
    ...
    return params.transactions.runInTransaction(async (dataAccess) => {
        ...
        const message = session.status === 'approved'
...
async function updateSourceAfterApprovedPayment(params) {
    ...
    if (item.sourceType === 'member_fee_charge' && item.sourceId) {
        ...
        await dataAccess.memberFeeCharges.update(item.sourceId, {
            status: 'paid',
    ...
    if (item.sourceType === 'tournament_registration' && item.sourceId) {
        await dataAccess.tournamentRegistrations.update(item.sourceId, {
            status: 'confirmed',
            paymentStatus: 'paid',
    ...
    if (item.sourceType === 'handicap_charge' && item.sourceId) {
        ...
        if (!handicapCharge?.incomeMovementId) {
            await dataAccess.handicapCharges.update(item.sourceId, {
...
export async function postMercadoPagoApprovedCheckout(params) {
    ...
    await params.dataAccess.mercadoPagoCheckoutSessions.update(params.session.id, {
        status: 'approved',
        paymentId: params.payment.paymentId,
        merchantOrderId: params.payment.merchantOrderId ?? params.session.merchantOrderId ?? null,
...
export async function processMercadoPagoPaymentUseCase(params) {
    ...
    return params.transactions.runInTransaction(async (dataAccess) => {
        ...
        if (status !== 'approved') {
            await dataAccess.mercadoPagoCheckoutSessions.update(session.id, {
                status,
                paymentId: params.payment.paymentId,
...
export async function recordMercadoPagoEventUseCase(params) {
    return params.transactions.runInTransaction(async (dataAccess) => {
        ...
        try {
            ...
            await params.transactions.runInTransaction(async (dataAccess) => {
                await dataAccess.mercadoPagoEvents.update(params.eventId, {
                    processed: true,
    ...
    return {
        items: itemsInput.map((rawItem) => {
            ...
            return {
                ...
                thirdPartyType: parseOptionalNullableString(item, 'thirdPartyType'),
...
export function parseGetMercadoPagoCheckoutStatusInput(payload) {
    const data = assertIsRecord(payload);
    return { sessionId: parseRequiredString(data, 'sessionId') };
}
export function parseMercadoPagoPaymentResponse(payload) {
    ...
    const feeAmountMinor = feeDetails.reduce((total, detail) => {
        ...
        const amount = amountToMinor(detail.amount);
...
//# sourceMappingURL=mercado-pago.use-cases.js.map