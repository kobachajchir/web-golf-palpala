...
import { assertIsRecord, ensureDirectivo, parseOptionalAccountingPeriod, parseOptionalAmountMinor, parseOptionalBoolean, parseOptionalIsoDate, parseOptionalNullableString, parseRequiredAccountingPeriod, parseRequiredIsoDate, parseRequiredString, parseRequiredStringArray, } from '../shared.js';
export async function createMacroDebitSettlementUseCase(params) {
    ...
    return params.transactions.runInTransaction(async (dataAccess) => {
        ...
        const resolvedMovements = movements.filter((movement) => movement !== null);
        ...
        for (const movement of resolvedMovements) {
            await dataAccess.financialMovements.update(movement.id, { settlementId }, actor.uid);
        }
...
export async function reconcileMacroSettlementUseCase(params) {
    ...
    return params.transactions.runInTransaction(async (dataAccess) => {
        ...
        await dataAccess.macroDebitSettlements.update(settlement.id, {
            grossAmountMinor,
            commissionAmountMinor,
            netAmountMinor,
...
export function parseCreateMacroDebitSettlementInput(payload) {
...
export function parseReconcileMacroSettlementInput(payload) {
    const data = assertIsRecord(payload);
    return {
...
//# sourceMappingURL=settlement.use-cases.js.map